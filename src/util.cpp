...
const char * const BITCOIN_CONF_FILENAME = "celer.conf";
const char * const BITCOIN_PID_FILENAME = "celerd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "celer" is a composite category enabling all Celer-related debug output
            if(ptrCategory->count(string("celer"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CelerCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CelerCore
    // Mac: ~/Library/Application Support/CelerCore
    // Unix: ~/.celercore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CelerCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CelerCore";
#else
    // Unix
    return pathRet / ".celercore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty celer.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override celer.conf
